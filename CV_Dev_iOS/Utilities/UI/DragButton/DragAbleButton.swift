               ...
               ZStack {
                   ...
                   Button(action:{
                       ...
                       ZStack{
                           ...
                           NormalWebView(isScroll: false, htmlString: liveChatData)
                               .frame(width: 48, height: 48)
                               .cornerRadius(24)
                               
                       }
                   }
                   .accessibilityIdentifier("liveChat_Button")
                   .animation(.default)
                       .position(self.dragAmount)
                   .highPriorityGesture(  // << to do no action on drag !!
                       DragGesture()
                        ...
                        .onEnded({
                            ...
                            if $0.location.y < HV*0.1{
                                self.dragAmount = CGPoint(x: $0.location.x, y: HV*0.1)
                            }
                            else if $0.location.y > HV*0.7{
                                self.dragAmount = CGPoint(x: $0.location.x, y: HV*0.7)
                            }
                            
                            if $0.location.x > WV*0.9{
                                self.dragAmount = CGPoint(x: WV*0.9, y: $0.location.y)
                            }
                            
                            if $0.location.x < -WV*0.3{
                                self.dragAmount = CGPoint(x: -WV*0.3, y: $0.location.y)
                            }
                        }))
                   
               }.frame(maxWidth: .infinity, maxHeight: .infinity) // full space
            .fullScreenCover(isPresented: $isChat) {
                Choose_Communicate(isChat: $isChat)
